...
class PIIScrubber:
    ...
    PII_PATTERNS = {
        ...
        "phone_us": (
            r"(?<!\d)(?:\+?1[\s.-]?)?"  # optional US country code
            r"(?:\(\d{3}\)|\d{3})"  # area code, bare or parenthesized
            r"[\s]*[-.]?[\s]*\d{3}"  # separator + prefix (middle 3 digits)
            r"[\s]*[-.]?[\s]*\d{4}(?!\d)"  # separator + last 4 digits
        ),
